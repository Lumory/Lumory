<template>
  <n-space vertical align="center" item-style="width: 100%">
    <h1>Wat zijn de eigenschappen die je zoekt in de student?</h1>
    ...
    <n-form ref="formRef" :model="formValue" size="large" :rules="rules" class="questionnaire-grid">
      ...
      <div class="questionnaire-description">
        <h3 class="questionnaire-description__header">Beschrijf de gewenste eigenschappen</h3>
        <n-form-item class="questionnaire-description__field" path="qualitiesInternDescription">
          <n-input
              v-model:value="formValue.qualitiesInternDescription"
  ...
  setup() {
    ...
    function onQuestionnaireItemCardClick(key, index) {
      if (itemRefs.value[index].selected === true) {
        ...
        formValue.value.qualitiesIntern = selectedItems
        return
      }
    ...
    function handleValidateClick() {
      if(formValue.value.qualitiesIntern.length < 1){
        this.message.error('Minimaal 1 antwoord is verplicht')
        throw new TypeError("QualitiesIntern is required")
      }
